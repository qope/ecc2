use crate::rlc::{RlcChip, RlcFixedTrace, RlcTrace};
use core::iter::once;
...
impl<'v, F: Field> KeccakChip<'v, F> {
    ...
    pub fn keccak_fixed_len(
        ...
        let output_assigned = gate.assign_witnesses(
            ...
            output_bytes
                .iter()
                .map(|b| Value::known(gate.get_field_element(*b as u64))),
        );
    ...
    pub fn keccak_var_len(
        ...
        let num_bytes = value_to_option(len.value())
            .map(|v| v.get_lower_32() as usize)
            .unwrap_or(min_len);
        ...
        let output_assigned = range.gate().assign_witnesses(
            ...
            output_bytes
                .iter()
                .map(|b| Value::known(range.gate().get_field_element(*b as u64))),
        );
    ...
    pub fn merkle_mountain_range(
        ...
        once(self.merkle_tree_root(ctx, gate, leaves))
            .chain(
                mountain_range_start_positions
                    .zip((0..max_depth).rev())
                    .map(|(start_idx, depth)| {
            ...
            self.fixed_len_queries
                ...
                .chain(
                    self.var_len_queries
                        .iter()
                        .map(|q| &q.input_bytes[..q.num_bytes]),
                ),
            rlc.gamma,
            ...
            self.var_len_queries.iter().zip(self.var_len_rlcs.iter())
        {
            ...
            let (table_input_rlc, table_output_rlc) = if min_keccak_f == max_keccak_f {
                running_num_squeezed =
                    gate.load_constant(ctx, gate.get_field_element(min_keccak_f as u64));
                (
                    input_rlcs[min_keccak_f - 1].clone(),
                    output_rlcs[min_keccak_f - 1].clone(),
                )
            } else {
                ...
                running_num_squeezed = gate.add(
                    ctx,
                    Existing(&running_num_squeezed),
                    Existing(&num_keccak_f),
                );
                ...
                let input_rlc = gate.select_from_idx(
                    ctx,
                    input_rlcs[min_keccak_f - 1..max_keccak_f]
                        .iter()
                        .map(Existing),
                    Existing(&keccak_idx),
                );
                let output_rlc = gate.select_from_idx(
                    ctx,
                    output_rlcs[min_keccak_f - 1..max_keccak_f]
                        .iter()
                        .map(Existing),
                    Existing(&keccak_idx),
                );
