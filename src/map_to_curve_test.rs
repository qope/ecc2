...
use halo2_ecc::{
    ...
    ecc::get_naf,
    fields::{fp::FpStrategy, FieldChip, FieldExtPoint},
};
...
pub fn assert_eq<'v>(a: &FieldExtPoint<CRTInteger<'v, Fr>>, x_dec: &str, y_dec: &str) {
...
pub fn sign0_fp2<'v>(
    ...
    let x = &a.coeffs[0];
    let sign_0 = sign0_fp(ctx, flex_chip, x);
    let y = &a.coeffs[1];
    let sign_1 = sign0_fp(ctx, flex_chip, y);
...
impl Circuit<Fr> for MyCircuit {
    ...
    fn configure(meta: &mut ConstraintSystem<Fr>) -> Self::Config {
        ...
        let flex_chip = FlexGateConfig::configure(meta, GateStrategy::Vertical, &[1], 1, 1, K);
        Self::Config { fp_chip, flex_chip }
    }
    ...
    fn synthesize(
        ...
        let flex_chip = config.flex_chip;
        ...
        layouter.assign_region(
            ...
            |region| {
                ...
                // // c1 = Fq2.g(Z);
                // // c2 = Z.div(Fq2.F(2, 0)).neg();
                let z = fp2_chip.load_constant(ctx, Fq2::one());
                let c1 = g(ctx, &fp2_chip, &z, &b_coeff);
                let two_const = fp2_chip.load_constant(ctx, Fq2::from(2));
                let c2 = fp2_chip.neg_divide(ctx, &z, &two_const);

                // // c3 = Fq2.sqrt(Fq2.neg(Fq2.g(Z).mul(Z.square().mul(Fq2.F(3, 0)))));
                // // c4 = Fq2.neg(Fq2.g(Z).mul(Fq2.F(4, 0)).div(Z.square().mul(Fq2.F(3, 0))));
                // - g(Z)*4/ ( Z^2 * 3)
                let three_const = fp2_chip.load_constant(ctx, Fq2::from(3));
                let z_sq = fp2_chip.mul(ctx, &z, &z);
                let z_sq_mul_3 = fp2_chip.mul(ctx, &z_sq, &three_const);
                let gz_z_sq_mul_3 = fp2_chip.mul(ctx, &c1, &z_sq_mul_3);
                let neg_gz_z_sq_mul_3 = fp2_chip.negate(ctx, &gz_z_sq_mul_3);
                let c3 = sqrt(ctx, &flex_chip, &fp_chip, &fp2_chip, &neg_gz_z_sq_mul_3);

                let four = fp2_chip.load_constant(ctx, Fq2::from(4));
                let g4 = fp2_chip.mul_no_carry(ctx, &c1, &four);
                let c4 = fp2_chip.neg_divide(ctx, &g4, &z_sq_mul_3);
                ...
                let tv3 = fp2_chip.carry_mod(ctx, &tv3);
                ...
                assert_eq(
                    &tv1,
                    "4804736240159840902444332968471109165811385375992205194248813196385537460451",
                    "21354383289599292899752590970982707403606157226632023085550280872824610935109",
                );

                assert_eq(
                    &tv2,
                    "17083506631679434319802072776786165922884925781305618468440224698259688748134",
                    "533859582239982322493814774274567685090153930665800577138757021820615273474",
                );

                assert_eq(
                    &tv3,
                    "8868811023676410356197716672529928364065445374923552007990315684689298294666",
                    "10503741753746556963102932711375861689622168702553914129638542687498922828906",
                );

                assert_eq(
                    &tv4,
                    "10356039868629348998401042642773708543550989364022522304275643197605088757804",
                    "14780309652284711482718429692162845108298299581354696723664233575735346851474",
                );

                let x1 = fp2_chip.sub_no_carry(ctx, &c2, &tv4);
                let x1 = fp2_chip.carry_mod(ctx, &x1);
                ...
                assert_eq(
                    &x1,
                    "588081567290288612722160229854929000797166214626389527068875749717524346487",
                    "7107933219554563739527976053094429980398011575943126939024804318909879357109",
                );
                ...
                assert_eq(
                    &x3,
                    "3496404167295669992210977349624622841745352654617441582255586303978679210363",
                    "14297107614410387406542565127150302328865820025361084022263427679286543406155",
                );
                ...
                let y_sign = sign0_fp2(ctx, &flex_chip, &y_imid);
                let u_sign = sign0_fp2(ctx, &flex_chip, &u);
